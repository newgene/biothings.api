...
class AsyncConnMixin:
    ...
    def update_headers(
        self, method, host, url, body=None, params=None, headers=None
    ):
...
if elasticsearch.VERSION[0] < 8:
    ...
    class AsyncConn(AsyncConnMixin, AIOHttpConnection):
        async def perform_request(
            self, method, url, params=None, body=None,
            timeout=None, ignore=(), headers=None
        ):
            headers = self.update_headers(method, self.host, url, body=body, params=params, headers=headers)
    ...
    from elastic_transport import RequestsHttpNode as _Conn

    class AsyncConnNode(AsyncConnMixin, AiohttpHttpNode):
        async def perform_request(
            self,
            method,
            target,
...
def get_es_client(hosts=None, async_=False, **settings):
    ...
    if settings.pop('aws', False):
        ...
        if elasticsearch.VERSION[0] < 8:
            settings.update(connection_class=_cc)
        else:
            settings.update(node_class=_Conn)
